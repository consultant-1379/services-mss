...
import com.ericsson.eniq.events.server.test.queryresults.mss.MssEventVolumeNoTypeResult;
 ...
 *
 */
public class MssEventVolumeWithPreparedTablesNoTypeTest extends
        MssTestsWithTemporaryTablesBaseTestCase<MssEventVolumeNoTypeResult> {
    ...
    private void createRawTableAndPopulateRawData(final String timestamp) throws Exception {
        ...
        //1. Create SMS Tables and populate data for test
        createSMSRawTablesAndPopulateData(this.connection, TEMP_EVENT_E_MSS_SMS_CDR_SUC_RAW,
                TEMP_EVENT_E_MSS_SMS_CDR_ERR_RAW, timestamp);

        //2. Create Location Service Tables and populate data for test
        createLocRawTablesAndPopulateData(conn1, TEMP_EVENT_E_MSS_LOC_SERVICE_CDR_SUC_RAW,
                TEMP_EVENT_E_MSS_LOC_SERVICE_CDR_ERR_RAW, timestamp);

        //3. Create Voice Tables and populate data for test
        createVoiceRawTablesAndPopulateData(conn2, TEMP_EVENT_E_MSS_VOICE_CDR_SUC_RAW,
    ...
    private void createAggTableAndPopulateAggData15MIN(final String timestamp) throws Exception {
        ...
        createSMSAggTablesAndPopulateData(this.connection, TEMP_SMS_EVNTSRC_EVENTID_SUC_15MIN,
                TEMP_SMS_EVNTSRC_EVENTID_ERR_15MIN, timestamp);

        //3. Create Location Service Tables and populate data for test
        createLocAggTablesAndPopulateData(conn1, TEMP_LOC_EVENT_SRC_SUM_SUC_15MIN, TEMP_LOC_EVENT_SRC_SUM_ERR_15MIN,
                timestamp);

        //4. Create Voice Tables and populate data for test
        createVoiceAggTablesAndPopulateData(conn2, TEMP_VOICE_EVENT_SRC_SUM_SUC_15MIN,
                TEMP_VOICE_EVENT_SRC_SUM_ERR_15MIN, TEMP_VOICE_EVENT_SRC_SUM_DROP_15MIN, timestamp);
    }

    private void createAggTableAndPopulateAggDataDAY(final String timestamp) throws Exception {
        createSMSAggTablesAndPopulateData(this.connection, TEMP_SMS_EVNTSRC_EVENTID_SUC_DAY,
                TEMP_SMS_EVNTSRC_EVENTID_ERR_DAY, timestamp);

        //3. Create Location Service Tables and populate data for test
        createLocAggTablesAndPopulateData(conn1, TEMP_LOC_EVENT_SRC_SUM_SUC_DAY, TEMP_LOC_EVENT_SRC_SUM_ERR_DAY,
                timestamp);

        //4. Create Voice Tables and populate data for test
        createVoiceAggTablesAndPopulateData(conn2, TEMP_VOICE_EVENT_SRC_SUM_SUC_DAY, TEMP_VOICE_EVENT_SRC_SUM_ERR_DAY,
                TEMP_VOICE_EVENT_SRC_SUM_DROP_DAY, timestamp);
    }
    ...
    private void validateResultFromTables(final List<MssEventVolumeNoTypeResult> results) {
        assertThat(results.size(), is(JSON_SIZE_RAW));
        final MssEventVolumeNoTypeResult result = results.get(JSON_RESULT_POSTATION_RAW);
        ...
        assertThat(result.getUnsuccessfulLocationRequests(), is(1));//18
    ...
    private void validateResultFromTables15MinAgg(final List<MssEventVolumeNoTypeResult> results) {
        assertThat(results.size(), is(JSON_SIZE_15MIN));
        final MssEventVolumeNoTypeResult result = results.get(JSON_RESULT_POSTATION_15MIN);
    ...
    private void validateResultFromTablesDayAgg(final List<MssEventVolumeNoTypeResult> results) {
        assertThat(results.size(), is(JSON_SIZE_DAY));
        final MssEventVolumeNoTypeResult result = results.get(JSON_RESULT_POSTATION_DAY);
    ...
    @Test
    public void testGetEventVolumeDataForNoTypeRaw() throws Exception {
        ...
        final List<MssEventVolumeNoTypeResult> summaryResult = getTranslator().translateResult(json,
                MssEventVolumeNoTypeResult.class);

        validateResultFromTables(summaryResult);
    }

    @Test
    public void testGetEventVolumeDataForNoType15MIN() throws Exception {
        ...
        validateResultFromTables15MinAgg(summaryResult);
    }

    @Test
    public void testGetEventVolumeDataForNoTypeDAY() throws Exception {
        ...
        final List<MssEventVolumeNoTypeResult> summaryResult = getTranslator().translateResult(json,
                MssEventVolumeNoTypeResult.class);
        validateResultFromTablesDayAgg(summaryResult);
    }
}
