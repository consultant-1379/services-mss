...
@Stateless
public class MssEventVolumeQuerySMSCreator extends MssEventVolumeQueryBaseCreator {

    @Override
    protected String getQuery() {
        if (!mssTableTypes.containsKey(MssTableType.SMS)
                || !mssTableTypes.get(MssTableType.SMS).booleanValue()) {
            return "";
        }
        final String dataSubsql = createDataTableQuery();
        eventVolumeParameters.put(DATA_TABLE_SUBSQL_VAR, dataSubsql);
        return templateUtils.getQueryFromTemplate(MSS_EVENT_VOLUME_SMS_TEMPLATE, eventVolumeParameters);
    }
    ...
    private String createDataTableQuery() {
        final String blockedSubsql = createOriginalTable(MssTableType.SMS, RawTableType.ERR,
                COMMON_COLUMNS_SMS, NO_OF_BLOCKED_CALLS, BLOCKED_TABLE_NAME);
        final String sucSubsql = createOriginalTable(MssTableType.SMS, RawTableType.SUC,
                COMMON_COLUMNS_SMS, NO_OF_SUCCESSES_CALLS, SUC_TABLE_NAME);
        eventVolumeParameters.put(COMMON_COLUMNS_VAR, COMMON_COLUMNS_SMS);
